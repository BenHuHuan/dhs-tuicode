...
估算器没有配置项。它有意使用一项固定启发式规则：每个 token 按四个字符估算，再加上角色、块与请求 envelope 字段的结构开销。图像块改用 DeepSeek vision 的尺寸 token 估算（缩放至约 800×800 总像素、8:1 宽高比上界、每张图像 384 token 上限）。任何配置键都会被拒绝；模型容量属于拥有精确提供方／模型路由的适配器，可通过 `ctx.llm.resolveModelInfo().context` 获取。
...
- **固定启发式规则是近似值**：没有可复用提供方用量的内容按字符数加结构开销计价（图像另按官方尺寸估算），而不是使用精确提供方 tokenizer 或请求 serializer。
