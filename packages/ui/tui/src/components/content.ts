...
function imageBytes(bytes: number): string {
...
/**
 * Format compact terminal-safe metadata for a durable image block.
 * @param block - Durable image content block to describe.
 * @returns A single-line terminal description.
 */
export function imageContentText(block: Extract<ContentBlock, { type: 'image' }>): string {
  ...
  return `[Image · ${format} · ${String(attachment.width)}×${String(attachment.height)} · ${imageBytes(attachment.bytes)}${name}]`
}
  ...
  for (const block of content) {
    switch (block.type) {
      ...
      case 'tool-result':
        ...
        break
      case 'image':
        parts.push(imageContentText(block))
        break
      default: {
