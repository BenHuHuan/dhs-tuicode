...
export interface QuestionQueueDeps extends ChatChannelDeps {
  ...
  /** Shared terminal UI used by the multiline custom-response editor. */
  readonly ui: TUI
  /** Release the terminal and edit a custom response out of process. */
  editExternally(draft: string): Promise<string>
}
    ...
    const show = (): void => {
      ...
      const session = overlayManager.open({
        ...
        create: () => new QuestionDialog(
          ...
          draft => deps.editExternally(draft),
          () => { deps.requestRender() },
        ),
        options: {
