 ...
 * neighbor, the pre-swap preflight, and the atomic channel swap itself.
...
export interface ResumeController {
  ...
  showResume(): void
  /** Resume one exact session id or unique exact title without opening the selector. */
  resume(reference: string, signal: AbortSignal): Promise<void>
}

/** The selected session is valid, but this embedding cannot replace the channel in place. */
class ResumeSwapUnavailableError extends Error {
  override readonly name = 'ResumeSwapUnavailableError'
}
...
export function createResumeController(deps: ResumeControllerDeps): ResumeController {
  ...
  /** Resolve an exact id first, then one unique exact folded title. */
  const resolveReference = async (
    listQuery: SessionQueryEngine,
    reference: string,
    signal: AbortSignal,
  ): Promise<ResumeCandidate> => {
    const records = await listQuery.listSessions(signal)
    signal.throwIfAborted()
    const idMatch = records.find(record => record.header.id === reference)
    if (idMatch !== undefined) return summarize(idMatch, undefined, undefined)

    const titles = await resolveTitles(listQuery, records, signal)
    signal.throwIfAborted()
    const matches: ResumeCandidate[] = []
    for (const [index, record] of records.entries()) {
      const resolution = titles[index] as TitleResolution
      if (resolution.title !== reference) continue
      matches.push(summarize(record, resolution.title, undefined))
    }
    if (matches.length === 0) {
      throw new Error(`No session matches id or title "${reference}".`)
    }
    if (matches.length > 1) {
      const ids = matches.map(match => match.record.header.id).sort().join(', ')
      throw new Error(`Session title "${reference}" is ambiguous (${ids}). Use a session id.`)
    }
    return matches[0] as ResumeCandidate
  }
  ...
  /**
   * Re-read every mutable precondition immediately before the channel swap.
   * This is where the one chosen log is fully read, replay-validated, checked
   * for a recorded workspace, and checked for a currently-available route —
   * the listing never does any of that.
   */
  const preflightResume = async (
    sessionId: SessionId,
    signal?: AbortSignal,
  ): Promise<SessionId> => {
    signal?.throwIfAborted()
    ...
    const record = (await query.listSessions(signal)).find(candidate => candidate.header.id === sessionId)
    signal?.throwIfAborted()
  ...
  /** Run the shared checked swap, optionally releasing a picker immediately before terminal drain. */
  const performResume = async (
    sessionId: SessionId,
    signal?: AbortSignal,
    beforeDrain?: () => Promise<void>,
  ): Promise<void> => {
    if (resumeInFlight) throw new Error('A resume swap is already in progress.')
    resumeInFlight = true
    try {
      const checkedId = await preflightResume(sessionId, signal)
      const swapResume = runtime.swapResume
      if (swapResume === undefined) {
        throw new ResumeSwapUnavailableError(
          'Session is resumable, but this host cannot swap it in place.',
        )
      }
      signal?.throwIfAborted()
      ...
      if (deps.isDisposed()) return
      // In-place swap: the host resumes the session without touching this
      // channel on rejection; on success it disposes this channel (terminal
      // release included) and remounts a fresh one for the resumed session.
      await swapResume(checkedId)
    } finally {
      resumeInFlight = false
    }
  }

  const swapFromPicker = async (candidate: ResumeCandidate, overlay: TuiOverlaySession): Promise<void> => {
    try {
      await performResume(candidate.record.header.id, undefined, async () => {
        await overlay.close()
      ...
      if (!deps.isDisposed()) {
        await overlay.close()
        resumeOverlay = undefined
        if (error instanceof ResumeSwapUnavailableError) {
          deps.appendNotice(error.message, 'warning')
        } else {
  ...
  return {
    async resume(reference: string, signal: AbortSignal): Promise<void> {
      if (deps.agentStatus() !== 'idle') {
        throw new Error('Resume requires the current turn to finish or be cancelled first.')
      }
      const listQuery = sessionQuery()
      if (listQuery === undefined) {
        throw new Error('Resume is not available: session query is not mounted.')
      }
      const candidate = await resolveReference(listQuery, reference, signal)
      await performResume(candidate.record.header.id, signal)
    },
    showResume(): void {
      ...
      const session = overlayManager.open({
        create: (host) => {
          picker = new ResumePicker(
            ...
            () => host.viewport.rows,
            palette,
            (candidate) => { void swapFromPicker(candidate, session) },
            () => { void session.close() },
          )
          return picker
        },
