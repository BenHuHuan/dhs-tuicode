...
import ToolRegistry, { type ToolDefinition, type ToolExecutionInput } from '@deepseek-ai/dsh-tools'
...
class ControllableShellProcess implements ShellProcess {
...
function resolvedShellSpec(request: ShellExecRequest): ShellExecSpec {
  return {
    command: request.command,
    workdir: request.workdir ?? '/executor-default',
...
describe('TUI config', () => {
  it('keeps the first-run notice optional and rejects an incomplete opt-in', () => {
    expect(Config({ welcome: 'overlay replaced the config' }).firstRunWelcome).toBeUndefined()
    expect(() => Config({ firstRunWelcome: {} } as never)).toThrow('$.firstRunWelcome expected')
  })
  ...
  it('defaults every direct-call TUI option', () => {
    expect(resolveTuiConfig(undefined)).toEqual({
      ...
      clipboardTextCommand: undefined,
      ...
      historyMaxSessions: 128,
      historyScanConcurrency: 4,
      ...
      directShellOutputMaxBytes: 64_000,
      directShellOutputRefreshMs: 50,
    ...
    expect(resolveTuiConfig({
      ...
      clipboardImageCommand: ['clipboard-reader', '--png'],
      clipboardTextCommand: ['clipboard-writer', '--stdin'],
      ...
      maxHistoryOptions: 6,
      historyMaxEntries: 321,
      historyMaxSessions: 17,
      historyScanConcurrency: 3,
      questionDialogWidth: 60,
      questionDialogMaxHeight: 14,
      modelDialogWidth: 64,
      modelDialogMaxHeight: 16,
      detailsDialogWidth: 44,
      settingsDialogWidth: 48,
      directShellOutputMaxBytes: 12_345,
      directShellOutputRefreshMs: 9,
    ...
    expect(resolveTuiUserSettings(undefined)).toEqual({
      showReasoning: true,
      externalEditorContext: false,
    })
    expect(resolveTuiUserSettings({ showReasoning: false, externalEditorContext: true })).toEqual({
      showReasoning: false,
      externalEditorContext: true,
    })
  })
...
describe('permission and plan mode controls', () => {
  async function configureModeServices(ctx: Context): Promise<void> {
    ...
    await ctx.plugin(PlanModeService, { section: 'Test plan-mode policy.' })
  }

  it('cycles safe modes, exposes pending state, and unlocks full access only after explicit selection', async () => {
    const result = await setup({
      omitInitialLifecycle: true,
      configureContext: configureModeServices,
    })

    expect(result.ctx.permissionPresets.current(result.session.events)).toBe('workspace-write')
    expect(result.terminal.output).toContain('⏵ Workspace Write')

    result.terminal.send('\x1b[Z')
    await tick()
    expect(result.ctx.planMode.get(result.agent)).toEqual({ active: true })
    expect(result.terminal.output).toContain('Mode: Plan.')
    expect(result.terminal.output).toContain('⏸ Plan')

    result.terminal.send('\x1bm')
    await tick()
    expect(result.ctx.planMode.get(result.agent)).toEqual({ active: false })
    expect(result.ctx.permissionPresets.current(result.session.events)).toBe('read-only')
    expect(result.terminal.output).toContain('Mode: Read Only.')

    result.terminal.send('/permission danger-full-access')
    result.terminal.send('\r')
    await tick()
    expect(result.ctx.permissionPresets.current(result.session.events)).toBe('danger-full-access')
    expect(result.terminal.output).toContain('⏵⏵ Full access')
    ...
    expect(result.ctx.planMode.get(result.agent)).toEqual({ active: false })
    expect(result.ctx.permissionPresets.current(result.session.events)).toBe('danger-full-access')
    expect(result.session.events.filter(event => event.type === 'permission/preset').map(event => event.data.preset)).toEqual([
      'workspace-write',
      'read-only',
      'danger-full-access',
      'read-only',
    ...
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output).toContain('Mode')
    expect(result.terminal.output).toContain('Full access')
    await dispose(result)
  })

  it('shows a pending plan target while a turn remains open', async () => {
    const result = await setup({ configureContext: configureModeServices })
    result.terminal.send('\x1b[Z')
    await tick()

    expect(result.ctx.planMode.get(result.agent)).toEqual({ active: false, pending: true })
    expect(result.terminal.output).toContain('Mode: Plan (applies from the next step).')
    expect(result.terminal.output).toContain('⏸ Plan (pending)')
    await dispose(result)
  })
})

describe('Ctrl+R prompt history', () => {
  const historyHeader = (id: string, cwd: string, createdAt: number): SessionHeader => ({
    version: 0,
    id: SessionId(id),
    cwd,
    createdAt,
  ...
  const historyEvents = (text: string, time: number): SessionEvent[] => [{
    type: 'tui/input',
    seq: 0,
    time,
    data: { text },
  ...
  it('records every accepted input form and collapses consecutive duplicates', async () => {
    const result = await setup()
    for (const input of ['same prompt', 'same prompt', '/help', '/skill:missing-skill']) {
      result.terminal.send(input)
      result.terminal.send('\r')
      await tick()
    }
    expect(result.session.events.filter(event => event.type === 'tui/input').map(event => event.data.text)).toEqual([
      'same prompt',
      '/help',
      '/skill:missing-skill',
    ])
    ...
    await tick()
    const rendered = result.terminal.output.slice(result.terminal.output.lastIndexOf('History search'))
    expect(rendered).toContain('/skill:missing-skill')
    expect(rendered).toContain('/help')
    expect(rendered).toContain('same prompt')
    result.terminal.send('\x1b')
    await dispose(result)
  })

  it('keeps a draft on cancel, inserts with Tab, and executes with Enter', async () => {
    const result = await setup({
      beforeMount(session) {
        session.append('tui/input', { text: 'deploy the current branch' })
        session.append('tui/input', { text: 'review the test failures' })
      },
    })

    result.terminal.send('draft stays')
    result.terminal.send('\x12')
    result.terminal.send('deploy')
    await tick()
    expect(result.terminal.output).toContain('deploy the current branch')
    result.terminal.send('\x1b')
    result.terminal.send('\r')
    expect(result.agent.sent.at(-1)).toEqual([{ type: 'text', text: 'draft stays' }])
    ...
    result.terminal.send('\r')
    expect(result.agent.sent.at(-1)).toEqual([{ type: 'text', text: 'review the test failures' }])

    result.terminal.send('\x12')
    result.terminal.send('deploy')
    result.terminal.send('\r')
    await tick()
    expect(result.agent.sent.at(-1)).toEqual([{ type: 'text', text: 'deploy the current branch' }])
    await dispose(result)
  })

  it('loads current-project history first and widens to all projects', async () => {
    const project = historyHeader('history-project', '/workspace', 200)
    const foreign = historyHeader('history-foreign', '/elsewhere', 300)
    const result = await setup({
      cwd: '/workspace',
      sessionPersistence: {
        list: async () => [foreign, project],
        load: async id => id === project.id
          ? { meta: project, events: historyEvents('project-only prompt', 210) }
          : { meta: foreign, events: historyEvents('foreign-only prompt', 310) },
      },
    })
    ...
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('project-only prompt')
    })
    let rendered = result.terminal.output.slice(result.terminal.output.lastIndexOf('History search'))
    expect(rendered).toContain('this project /workspace')
    expect(rendered).not.toContain('foreign-only prompt')

    result.terminal.send('\x13')
    await tick()
    rendered = result.terminal.output.slice(result.terminal.output.lastIndexOf('History search'))
    expect(rendered).toContain('all projects')
    expect(rendered).toContain('foreign-only prompt')
    result.terminal.send('foreign-only')
    result.terminal.send('\t')
    await tick()
    result.terminal.send('\r')
    expect(result.agent.sent.at(-1)).toEqual([{ type: 'text', text: 'foreign-only prompt' }])
    await dispose(result)
  })
})
...
describe('goodbye message and /resume', () => {
  ...
  it('opens a newest-active-first searchable selector and Esc clears before cancelling', async () => {
    ...
    const swap = vi.fn<NonNullable<TuiRuntime['swapResume']>>()
    const result = await setup({
      cwd: '/workspace',
      swapResume: swap,
      sessionPersistence: {
  ...
  it('resumes directly by exact id or unique exact title and exposes /continue as the same path', async () => {
    const byId = header('opaque-direct-id', 10, '/workspace')
    const byTitle = header('title-target-id', 20, '/elsewhere')
    const duplicateA = header('duplicate-a', 30, '/workspace')
    const duplicateB = header('duplicate-b', 40, '/workspace')
    const records = [byId, byTitle, duplicateA, duplicateB]
    const events = new Map<SessionId, SessionEvent[]>([
      [byId.id, resumeEvents('ID target title')],
      [byTitle.id, resumeEvents('Unique release train')],
      [duplicateA.id, resumeEvents('Duplicate title')],
      [duplicateB.id, resumeEvents('Duplicate title')],
    ])
    const swap = vi.fn<NonNullable<TuiRuntime['swapResume']>>()
    const result = await setup({
      cwd: '/workspace',
      ...
      sessionPersistence: {
        ...
        load: async (id) => {
          ...
          if (record === undefined || log === undefined) throw new Error(`missing fixture ${id}`)
          return { meta: record, events: log }
        },
      },
    })

    result.terminal.send('/resume opaque-direct-id')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(swap).toHaveBeenCalledWith(byId.id) })

    result.terminal.send('/continue Unique release train')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(swap).toHaveBeenCalledWith(byTitle.id) })
    expect(swap).toHaveBeenCalledTimes(2)
    expect(result.terminal.output).not.toContain('Resume session')

    result.terminal.send('/resume Duplicate title')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain(
        'Session title "Duplicate title" is ambiguous (duplicate-a, duplicate-b).',
      )
      expect(result.terminal.output).toContain('Use a session id.')
    })
    expect(swap).toHaveBeenCalledTimes(2)

    result.terminal.send('/continue missing reference')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain(
        'No session matches id or title "missing reference".',
      )
    })
    expect(swap).toHaveBeenCalledTimes(2)
  ...
  it('falls back to assistant provenance and header creation time for sparse logs', async () => {
    ...
    result.terminal.send('\r')
    await tick(); await tick()
    expect(result.terminal.output).toContain('cannot swap it in place')
    await dispose(result)
  })

  it('flushes and invokes one host swap for the selected session; a rejected swap keeps this channel', async () => {
    const target = header('target-session', 10, '/workspace')
    const swap = vi.fn<NonNullable<TuiRuntime['swapResume']>>(() => Promise.reject(new Error('test host retained process')))
    const result = await setup({
      cwd: '/workspace',
      swapResume: swap,
      sessionPersistence: {
    ...
    expect(swap).toHaveBeenCalledWith(target.id)
    // The channel never releases the terminal itself: on rejection the host
    // keeps this channel alive, so the terminal stays up.
    expect(result.terminal.stopped).toBe(0)
    expect(result.terminal.output).toContain('Resume failed: test host retained process')
    await dispose(result)
  })
  ...
  it('does not restart the terminal when a pending host rejects during disposal', async () => {
    ...
    const swap = vi.fn<NonNullable<TuiRuntime['swapResume']>>(() => host.promise)
    const result = await setup({
      cwd: '/workspace',
      swapResume: swap,
      sessionPersistence: {
  ...
  // A record that changes between listing and preflight is no longer a
  // rejection: resume targets whatever identity the record carries at
  // preflight, so the swap receives the RE-READ id rather than the listed one.
  it('swaps with the id re-read at preflight, not the one listed', async () => {
  ...
  it('lists other workspaces only in the all-workspaces scope and swaps them', async () => {
    ...
    result.terminal.send('\r')
    await tick(); await tick()
    expect(swap).toHaveBeenCalledWith(foreign.id)
    await dispose(result)
  })
  ...
  it('admits only one swap while the selected preflight is pending', async () => {
    const target = header('single-swap', 10, '/workspace')
    ...
    const result = await setup({
      ...
      sessionPersistence: {
        list: async () => [target],
        load: () => ++loads === 1
          ? Promise.resolve({ meta: target, events: resumeEvents('Single swap') })
          : preflight.promise,
      },
    })
    result.terminal.send('/resume')
    result.terminal.send('\r')
    await tick(); await tick()
    result.terminal.send('Single swap')
    result.terminal.send('\r')
    result.terminal.send('\r')
    await tick()
    preflight.resolve({ meta: target, events: resumeEvents('Single swap') })
    await tick(); await tick()
    expect(loads).toBe(2)
    await dispose(result)
  })
  ...
  it('warns without releasing the terminal when the host cannot swap', async () => {
    ...
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Session is resumable, but this host cannot swap it in place')
    })
    expect(result.terminal.stopped).toBe(0)
    await dispose(result)
  })
...
describe('pi-tui chat lifecycle and transcript', () => {
  it('restores durable goal phase without implying automatic continuation', async () => {
    ...
    const result = await setup({
      beforeMount(session) {
        // The strict fold projects goal state from the durable `goal/change`
        // log alone; the attributed message's `change` field is presentation
        // only, so the replay must see the change event first.
        session.append('goal/change', change)
        session.append('user/message', createUserMessage({
          content: [{ type: 'text', text: 'Goal restored (active) with automatic continuation disarmed. Use /goal resume to continue.' }],
          source: {
            kind: 'goal',
            goalId: change.goal.id,
            revision: change.goal.revision,
            // The fold admits rounds from 1: a round-0 source would be rejected
            // as `not the next admitted round` of the restored active goal.
            round: 1,
            change,
          },
        }), { surfaceOp: 'append' })
  ...
  it('renders its header, footer, replay, streaming answer, todos, and status', async () => {
    ...
    // The rework folded queued steering into plain user messages: the steering
    // note renders under the shared `You` label rather than a `Steering` badge.
  ...
  it('counts failed and recovered request usage once per step', async () => {
    ...
    result.session.append('llm/retry', {
      ...
      maxRetries: 2,
      delayMs: 500,
      retryId: RetryId('retry-temporary'),
  ...
  it('retracts a failed live stream and renders its durable retry status', async () => {
    ...
    result.session.append('llm/retry', {
      ...
      maxRetries: 2,
      delayMs: 500,
      retryId: RetryId('retry-rate-limited'),
    ...
    result.session.append('llm/retry', {
      ...
      maxRetries: 2,
      delayMs: 1_000,
      retryId: RetryId('retry-failed-chunks'),
    ...
    result.session.append('llm/retry', {
      ...
      retry: 1,
      delayMs: 2_000,
      retryId: RetryId('retry-unlimited'),
  ...
  it('badges queued steering on the prompt context and clears it as each drains', async () => {
    ...
    // A bare step/start is lifecycle state, not an empty assistant transcript row.
    ...
    const drainSteering = (text: string): void => {
      ...
      if (id !== undefined) {
        result.ctx.emit('agent/inbox/claimed', {
          agent: result.agent,
    ...
    result.terminal.output = ''
    result.ctx.emit('agent/inbox/inserted', {
      agent: other,
      message: freezeMessage({
        id: MessageId('stub'),
        role: 'user',
    ...
    // A bare session append has no inbox identity and therefore cannot
    // consume a pending slot by itself.
    result.terminal.output = ''
    result.session.append('user/message', createUserMessage({
    ...
    // the badge alone.
    result.ctx.emit('agent/inbox/claimed', { agent: other, message: discarded[0]!, turn: 1 })
    result.ctx.emit('agent/inbox/claimed', {
      agent: result.agent,
      message: freezeMessage({
    ...
    result.ctx.emit('agent/inbox/discarded', {
      agent: result.agent,
      message: freezeMessage({
        id: MessageId('never-queued'),
        role: 'user',
    ...
    for (const message of discarded) {
      result.ctx.emit('agent/inbox/discarded', { agent: result.agent, message })
    }
    await tick()
    expect(result.terminal.output).not.toContain('queued')
  ...
  it('reports a failed standalone compaction when its live bracket closes', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('compact-1'), turn: null })
    result.terminal.output = ''
    result.session.append('compaction/end', { compactionId: CompactionId('compact-1'), turn: null, error: 'summary failed' })
    await tick()
  ...
  it('does not show compaction progress for a resumed orphaned start', async () => {
    const result = await setup({
      ...
      now: () => 1_000,
      beforeMount(session) {
        session.append('compaction/start', { compactionId: CompactionId('compact-1'), turn: null })
      },
    })
  ...
  it('tracks steering drains without a running status line', async () => {
    ...
    result.ctx.emit('agent/inbox/inserted', { agent: result.agent, message: freezeMessage({
      id: MessageId('stub'),
      role: 'user',
  ...
  it('refreshes the running turn timing on its own timer', async () => {
    ...
    try {
      ...
      refresh()
      await tick()
      expect(result.terminal.output).toContain('Thinking 1.0s')
    } finally {
      if (result !== undefined) await dispose(result)
  ...
  it('shows aggregate and expanded context pressure from the authoritative token views', async () => {
    const result = await setup({
      ...
      contextMeasurement: {
        ...
        baseline: {
          kind: 'usage',
          tokens: 40_000,
          usage: { inputTokens: 39_000, outputTokens: 1_000 },
        },
        surfaceDeltaTokens: 2_000,
        totalTokens: 42_000,
        surfaceTokens: 20_000,
        nodes: [
          { seq: 2, tokens: 8_000 },
          { seq: 3, tokens: 12_000 },
        ],
      ...
      beforeMount(session) {
        appendUser(session, 'Inspect context usage.')
        appendAssistant(session, [{ type: 'text', text: 'Context inspected.' }])
      },
    })

    result.terminal.send('/context all')
    result.terminal.send('\r')
    await tick()

    expect(result.terminal.output).toContain('Context usage')
    expect(result.terminal.output).toContain('[█████░░░░░░░░░░░] 33% used (42,000 / 128,000)')
    expect(result.terminal.output).toContain('Provider usage anchored + live surface delta')
    expect(result.terminal.output).toContain('≈ 10,000 tokens')
    expect(result.terminal.output).toContain('≈ 12,000 tokens')
    expect(result.terminal.output).toContain('≈ 20,000 tokens')
    expect(result.terminal.output).toContain('2 model-visible messages')
    expect(result.terminal.output).toContain('User prompt · ≈ 8,000 tokens')
    expect(result.terminal.output).toContain('Assistant response · ≈ 12,000 tokens')

    const invalid = result.terminal.output.length
    result.terminal.send('/context verbose')
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output.slice(invalid)).toContain('Usage: /context [all]')
    await dispose(result)
  })

  it('labels unavailable context capacity and composition without inventing a breakdown', async () => {
    const result = await setup({
      omitInitialLifecycle: true,
      contextTokens: 7,
      agentOptions: {},
    ...
    result.terminal.send('/context')
    result.terminal.send('\r')
    await tick()

    expect(result.terminal.output).toContain('7 used · capacity unknown')
    expect(result.terminal.output).toContain('Estimated request pressure')
    expect(result.terminal.output).toContain('Unavailable in this runtime composition')
    expect(result.terminal.output).not.toContain('Surface items')
    await dispose(result)
  })
  ...
  it('shows detailed session diagnostics while the agent is running', async () => {
    ...
    // 6 domain events + exact TUI input + /status command/run (open turn: joined directly).
    expect(result.terminal.output).toContain('running · 8 events · 1 turn · 1 step · 2 tool calls')
  ...
  it('labels unavailable status diagnostics without inventing values', async () => {
    ...
    // Exact TUI input and /status command/run land directly on the empty log; no turn wraps them.
    expect(result.terminal.output).toContain('idle · 2 events · 0 turns · 0 steps · 0 tool calls')
  ...
  it('/config persists live settings and the external editor reads the committed value', async () => {
    let settings = { showReasoning: true, externalEditorContext: false }
    const updateSettings = vi.fn<NonNullable<TuiRuntime['updateSettings']>>(async (patch) => {
      settings = { ...settings, ...patch }
      return settings
    })
    const editText = vi.fn<NonNullable<TuiRuntime['editText']>>(async ({ draft }) => draft)
    const result = await setup({
      readSettings: () => settings,
      updateSettings,
      editText,
    })
    appendAssistant(result.session, [{ type: 'text', text: 'settings context reply' }])
    await tick()

    const opened = result.terminal.output.length
    result.terminal.send('/config')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output.slice(opened)).toContain('TUI settings') })
    expect(result.terminal.output.slice(opened)).toContain('Reasoning blocks')
    expect(result.terminal.output.slice(opened)).toContain('External editor context')
    expect(result.terminal.output.slice(opened)).toContain('omitted')

    result.terminal.send('\x1b[B')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(updateSettings).toHaveBeenCalledWith({ externalEditorContext: true }) })
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Saved external editor context.') })
    expect(settings.externalEditorContext).toBe(true)
    result.terminal.send('\x1b')
    result.terminal.send('configured draft')
    result.terminal.send('\x07')
    await vi.waitFor(() => { expect(editText).toHaveBeenCalledOnce() })
    expect(editText).toHaveBeenCalledWith({
      draft: 'configured draft',
      previousResponse: 'settings context reply',
    })

    await dispose(result)
  })

  it('/config reports a missing settings writer without optimistically changing the value', async () => {
    const result = await setup({
      readSettings: () => ({ showReasoning: true, externalEditorContext: false }),
    })
    result.terminal.send('/config')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('TUI settings') })
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Save failed: this runtime has no writable user-settings service')
    })
    expect(result.terminal.output).toContain('shown')
    await dispose(result)
  })

  it('/copy selects persistent visible responses newest-first and validates N', async () => {
    const writeClipboardText = vi.fn<NonNullable<TuiRuntime['writeClipboardText']>>(() => Promise.resolve())
    const result = await setup({ writeClipboardText })
    appendAssistant(result.session, [{ type: 'text', text: 'older ' }, { type: 'text', text: 'answer\n' }])
    appendAssistant(result.session, [{ type: 'reasoning', text: 'private reasoning' }])
    appendAssistant(result.session, [{ type: 'text', text: '\nnewest answer\n' }])
    await tick()

    result.terminal.send('/copy')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(writeClipboardText).toHaveBeenCalledTimes(1) })
    expect(writeClipboardText.mock.calls[0]?.[0]).toMatchObject({
      text: '\nnewest answer\n',
      cwd: '/workspace',
    })
    await vi.waitFor(() => {
    ...
    for (const command of ['/copy 0', '/copy -1', '/copy 1.5', '/copy two', '/copy 3']) {
      result.terminal.send(command)
      result.terminal.send('\r')
      await tick()
    }
    expect(result.terminal.output).toContain('Usage: /copy [N] (N must be a positive integer)')
    expect(result.terminal.output).toContain('Assistant response #3 is unavailable; this session has 2 visible responses.')
    expect(writeClipboardText).toHaveBeenCalledTimes(2)
    await dispose(result)
  })

  it('/copy opens a fenced-code picker, copies code or the full reply, and cancels cleanly', async () => {
    const writeClipboardText = vi.fn<NonNullable<TuiRuntime['writeClipboardText']>>(() => Promise.resolve())
    const result = await setup({ writeClipboardText })
    const response = [
      'Use this:',
    ...
    appendAssistant(result.session, [{ type: 'text', text: response }])
    await tick()
    ...
    const openPicker = async (): Promise<void> => {
      ...
      result.terminal.send('/copy')
      result.terminal.send('\r')
      await vi.waitFor(() => { expect(result.terminal.output.slice(from)).toContain('Copy response #1') })
    }

    await openPicker()
    expect(result.terminal.output).toContain('Full response')
    expect(result.terminal.output).toContain('Code block 1')
    ...
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(writeClipboardText).toHaveBeenCalledTimes(2) })
    expect(writeClipboardText.mock.calls[1]?.[0].text).toBe(response)
  ...
  it('/copy picker writes the highlighted target to a path with explicit overwrite confirmation', async () => {
    const writeClipboardText = vi.fn<NonNullable<TuiRuntime['writeClipboardText']>>(() => Promise.resolve())
    const writeTextFile = vi.fn<NonNullable<TuiRuntime['writeTextFile']>>(({ path, overwrite }) => {
      ...
      return Promise.resolve(overwrite
        ? { kind: 'written', path: resolvedPath }
        : path === 'existing.ts'
          ? { kind: 'exists', path: resolvedPath }
          : { kind: 'written', path: resolvedPath })
    })
    const result = await setup({ writeClipboardText, writeTextFile })
    appendAssistant(result.session, [{
      type: 'text',
      text: ['Example:', '```ts', 'const answer = 42', '```'].join('\n'),
    }])
    await tick()

    const openCodeWrite = async (): Promise<void> => {
      const from = result.terminal.output.length
      result.terminal.send('/copy')
      result.terminal.send('\r')
      await vi.waitFor(() => { expect(result.terminal.output.slice(from)).toContain('w write file') })
      result.terminal.send('\x1b[B')
      result.terminal.send('w')
      await vi.waitFor(() => { expect(result.terminal.output.slice(from)).toContain('Write response to file') })
    }

    await openCodeWrite()
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Enter a file path before writing.')
    })
    result.terminal.send('answer.ts')
    result.terminal.send('\r')
    ...
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Wrote code block 1 from response #1 to /workspace/answer.ts.')
    })

    await openCodeWrite()
    result.terminal.send('existing.ts')
    const firstOverwrite = result.terminal.output.length
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(firstOverwrite)).toContain('Overwrite existing file?')
    })
    expect(result.terminal.output.slice(firstOverwrite)).toContain('/workspace/existing.ts')
  ...
  it('/copy picker degrades for a missing file writer and cancels the path prompt cleanly', async () => {
    const writeClipboardText = vi.fn<NonNullable<TuiRuntime['writeClipboardText']>>(() => Promise.resolve())
    const unavailable = await setup({ writeClipboardText })
    appendAssistant(unavailable.session, [{ type: 'text', text: '```txt\nwrite me\n```' }])
    unavailable.terminal.send('/copy')
    unavailable.terminal.send('\r')
    await vi.waitFor(() => { expect(unavailable.terminal.output).toContain('Copy response #1') })
    unavailable.terminal.send('w')
    await vi.waitFor(() => {
      expect(unavailable.terminal.output).toContain(
        'File write is unavailable because this runtime has no response file writer.',
      )
    })
    expect(writeClipboardText).not.toHaveBeenCalled()
    await dispose(unavailable)

    const writeTextFile = vi.fn<NonNullable<TuiRuntime['writeTextFile']>>()
    const cancelled = await setup({ writeClipboardText, writeTextFile })
    appendAssistant(cancelled.session, [{ type: 'text', text: '```txt\nwrite me\n```' }])
    cancelled.terminal.send('/copy')
    cancelled.terminal.send('\r')
    await vi.waitFor(() => { expect(cancelled.terminal.output).toContain('Copy response #1') })
    cancelled.terminal.send('w')
    await vi.waitFor(() => { expect(cancelled.terminal.output).toContain('Write response to file') })
    cancelled.terminal.send('\x1b')
    await tick()
    expect(writeTextFile).not.toHaveBeenCalled()
    await dispose(cancelled)
  })

  it('/copy aborts an in-flight response file writer on disposal', async () => {
    let writerSignal: AbortSignal | undefined
    const writeTextFile = vi.fn<NonNullable<TuiRuntime['writeTextFile']>>(({ signal }) => {
      writerSignal = signal
    ...
    appendAssistant(result.session, [{ type: 'text', text: '```txt\nslow write\n```' }])
    result.terminal.send('/copy')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Copy response #1') })
    result.terminal.send('w')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Write response to file') })
    result.terminal.send('slow.txt')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(writeTextFile).toHaveBeenCalledOnce() })
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Writing assistant response') })
    await dispose(result)
    expect(writerSignal?.aborted).toBe(true)
  })

  it('/copy degrades visibly and aborts an in-flight host writer on disposal', async () => {
    const unavailable = await setup()
    appendAssistant(unavailable.session, [{ type: 'text', text: 'copy me' }])
    unavailable.terminal.send('/copy')
    unavailable.terminal.send('\r')
    await vi.waitFor(() => {
      expect(unavailable.terminal.output).toContain(
        'Clipboard copy is unavailable because this runtime has no clipboard text writer.',
      )
    })
    await dispose(unavailable)

    const empty = await setup({ writeClipboardText: () => Promise.resolve() })
    empty.terminal.send('/copy')
    empty.terminal.send('\r')
    await vi.waitFor(() => {
      expect(empty.terminal.output).toContain('No visible assistant response is available to copy.')
    })
    await dispose(empty)

    let writerSignal: AbortSignal | undefined
    const writeClipboardText = vi.fn<NonNullable<TuiRuntime['writeClipboardText']>>(({ signal }) => {
      writerSignal = signal
    ...
    const pending = await setup({ writeClipboardText })
    appendAssistant(pending.session, [{ type: 'text', text: 'slow copy' }])
    pending.terminal.send('/copy')
    pending.terminal.send('\r')
    await vi.waitFor(() => { expect(writeClipboardText).toHaveBeenCalledOnce() })
    await vi.waitFor(() => {
      expect(pending.terminal.output).toContain('Copying assistant response')
    })
  ...
  it('applies an externally committed reasoning-display update to a mounted channel', async () => {
    let settings = { showReasoning: true, externalEditorContext: false }
    const result = await setup({ readSettings: () => settings })
    appendAssistant(result.session, [{ type: 'reasoning', text: 'live settings reasoning marker' }])
    await tick()
    expect(result.terminal.output).toContain('live settings reasoning marker')

    settings = { ...settings, showReasoning: false }
    const updated = result.terminal.output.length
    result.controller.updateSettings(settings)
    await tick()
    expect(result.terminal.output.slice(updated)).not.toContain('live settings reasoning marker')
    await dispose(result)
  })
  ...
  it('sends, steers, handles commands, global keys, and disposed-agent input', async () => {
    ...
    result.agent.status = 'idle'
    for (const command of ['/details reasoning off expanded', '/help', '/palette', '/reload']) {
      result.terminal.send(command)
      result.terminal.send('\r')
      await tick()
    }
  ...
  it('keeps Ctrl+D editor-local with text and confirms empty-prompt exits', async () => {
    let currentTime = 0
    const result = await setup({ now: () => currentTime })

    result.terminal.send('abcd')
    result.terminal.send('\x1b[D')
    result.terminal.send('\x04')
    result.terminal.send('\r')
    expect(result.agent.sent).toEqual([[{ type: 'text', text: 'abc' }]])
    ...
    // An unrelated key disarms the confirmation and still reaches the editor.
    result.terminal.send('x')
    result.terminal.send('\x03')
    result.terminal.send('\x04')
    expect(result.exit).not.toHaveBeenCalled()

    // A press outside the 800 ms window rearms instead of exiting.
    currentTime = 801
    result.terminal.send('\x04')
    expect(result.exit).not.toHaveBeenCalled()
    result.terminal.send('\x04')
  ...
  it('redraws once on Ctrl+L, preserves the draft, and rearms after two seconds', async () => {
    let currentTime = 0
    const swap = vi.fn<NonNullable<TuiRuntime['swapFresh']>>()
    const result = await setup({ now: () => currentTime, swapFresh: swap })

    result.terminal.send('draft')
    result.terminal.send('\x0c')
    await tick()
    expect(result.terminal.output).toContain('Press Ctrl+L again to run /clear')
    expect(swap).not.toHaveBeenCalled()
    ...
    result.terminal.send('\r')
    expect(result.agent.sent).toEqual([[{ type: 'text', text: 'draftZ' }]])
    await dispose(result)
  })

  it('edits a prompt with Ctrl+G while releasing the terminal and optionally including reply context', async () => {
    const pending = Promise.withResolvers<string>()
    const editText = vi.fn<NonNullable<TuiRuntime['editText']>>(() => pending.promise)
    const result = await setup({
      config: { externalEditorContext: true },
      editText,
    })
    appendAssistant(result.session, [{ type: 'text', text: 'latest assistant reply' }])
    await tick()
    const eventCount = result.session.events.length

    result.terminal.send('draft prompt')
    result.terminal.send('\x07')
    await vi.waitFor(() => { expect(editText).toHaveBeenCalledOnce() })
    expect(editText).toHaveBeenCalledWith({
      draft: 'draft prompt',
      previousResponse: 'latest assistant reply',
    })
    expect(result.terminal.drainInput).toHaveBeenCalledWith(100, 20)
    expect(result.terminal.stopped).toBe(1)
    expect(result.terminal.started).toBe(1)
    expect(result.session.events).toHaveLength(eventCount)

    // FakeTerminal can inject while stopped; the channel must consume it so
    // keystrokes intended for the external editor never leak into the draft.
    result.terminal.send('must not leak')
    pending.resolve('edited\nprompt')
    await vi.waitFor(() => { expect(result.terminal.started).toBe(2) })
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'edited\nprompt' }])
    await dispose(result)
  })

  it('supports Ctrl+X Ctrl+E, passes unrelated chord followers, and preserves a draft on failure', async () => {
    const editText = vi.fn<NonNullable<TuiRuntime['editText']>>()
      .mockResolvedValueOnce('from external editor')
      .mockRejectedValueOnce(new Error('editor crashed'))
    const result = await setup({ editText })

    result.terminal.send('a')
    result.terminal.send('\x18')
    result.terminal.send('b')
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'ab' }])

    result.terminal.send('\x18')
    result.terminal.send('\x05')
    await vi.waitFor(() => { expect(editText).toHaveBeenCalledTimes(1) })
    expect(editText).toHaveBeenLastCalledWith({ draft: '' })
    await vi.waitFor(() => { expect(result.terminal.started).toBe(2) })
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'from external editor' }])

    result.terminal.send('keep this draft')
    result.terminal.send('\x18\x05')
    await vi.waitFor(() => { expect(editText).toHaveBeenCalledTimes(2) })
    await vi.waitFor(() => { expect(result.terminal.output).toContain('External editor failed: editor crashed') })
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'keep this draft' }])
    expect(result.terminal.stopped).toBe(2)
    expect(result.terminal.started).toBe(3)
    await dispose(result)
  })

  it('requires two Ctrl+X Ctrl+K chords and stops only directly owned running subagent jobs', async () => {
    ...
    const result = await setup({
      ...
      configureContext(ctx) {
        ctx.provide('jobs', {
          list: () => [
            { id: JobId('subagent-1'), kind: 'subagent', label: 'owned', ownerSession: SessionId('main-session'), status: 'running' },
            { id: JobId('bash-1'), kind: 'bash', label: 'shell', ownerSession: SessionId('main-session'), status: 'running' },
            { id: JobId('subagent-2'), kind: 'subagent', label: 'foreign', ownerSession: SessionId('other'), status: 'running' },
            { id: JobId('subagent-3'), kind: 'subagent', label: 'unowned', status: 'running' },
          ] as never,
          kill,
        } as never)
    ...
    result.terminal.send('\x18')
    result.terminal.send('\x0b')
    await tick()
    expect(result.terminal.output).toContain(
      'Press Ctrl+X Ctrl+K again within 3s to stop all running background subagents',
    )
    expect(kill).not.toHaveBeenCalled()

    // An unrelated follower cancels the confirmation and still reaches the
    // prompt; the next complete chord therefore arms instead of stopping.
    result.terminal.send('a')
    result.terminal.send('\x18\x0b')
    await tick()
    expect(kill).not.toHaveBeenCalled()
    ...
    expect(kill).toHaveBeenCalledWith(
      JobId('subagent-1'),
      result.agent,
      'Stopped from the TUI with Ctrl+X Ctrl+K',
    )
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Stopping 1 background subagent…')
    })

    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'a' }])
    await dispose(result)
  })

  it('reports an unavailable stop-all chord without changing the prompt', async () => {
    const result = await setup()
    result.terminal.send('draft')
    result.terminal.send('\x18\x0b')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Background subagent control is unavailable')
      expect(result.terminal.output).toContain('service.')
    })
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'draft' }])
    await dispose(result)
  })

  it('degrades explicitly when an embedding has no external-editor boundary', async () => {
    const result = await setup()
    result.terminal.send('still here')
    result.terminal.send('\x07')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('External editor failed: this host does not provide an external editor')
    })
    expect(result.terminal.stopped).toBe(0)
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'still here' }])
    await dispose(result)
  })

  it('flushes and requests one fresh-session swap on double Ctrl+L or /clear', async () => {
    const flush = vi.fn()
    const swap = vi.fn<NonNullable<TuiRuntime['swapFresh']>>(
    ...
    const initialEvents = result.session.events.length
    ...
    expect(result.session.events).toHaveLength(initialEvents)
    expect(result.terminal.output).toContain('Clear failed: test host retained the current session')

    // A rejected atomic swap leaves the old editor and agent usable.
    result.terminal.send('still usable')
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'still usable' }])
  ...
  it('renames the durable session and lets /clear name it before a fresh-session swap', async () => {
    ...
    const swap = vi.fn<NonNullable<TuiRuntime['swapFresh']>>(async () => {
      titlesAtSwap.push(foldSessionTitle(result.session.events)?.title)
    })
    const result = await setup({
      swapFresh: swap,
      async configureContext(ctx) {
        await ctx.plugin(ToolRegistry)
        await ctx.plugin(SessionTitleService, SESSION_TITLE_CONFIG)
      },
    })

    result.terminal.send('/rename   Release\t candidate  ')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Session renamed: Release candidate.')
    })
    expect(foldSessionTitle(result.session.events)).toMatchObject({
      title: 'Release candidate',
      messageSeqs: [],
      source: { kind: 'user' },
    })
    expect(result.terminal.title).toBe('Release candidate — DeepSeek Harness')
    const explicitRun = result.session.events.find(event =>
      event.type === 'command/run' && event.data.name === 'rename')
    expect(explicitRun?.data).not.toHaveProperty('args')
    expect(result.agent.sent).toEqual([])
    expect(result.agent.steered).toEqual([])

    // A bare rename deliberately unpins the explicit title and derives a new
    // fallback from eligible conversation history when no provider is mounted.
    result.session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'Generate this session name from history please' }],
      source: { kind: 'user' },
    }), { surfaceOp: 'append' })
    ...
    await vi.waitFor(() => {
      expect(foldSessionTitle(result.session.events)).toMatchObject({
        title: 'Generate this session name from',
        source: { kind: 'fallback' },
      })
      expect(result.terminal.output).toContain('Session renamed: Generate this session name from.')
    })

    const titleCount = result.session.events.filter(event => event.type === 'session/title').length
    result.terminal.send('/rename \u200B')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain(
        'session title must contain visible characters. Usage: /rename <name>',
      )
    })
    expect(result.session.events.filter(event => event.type === 'session/title')).toHaveLength(titleCount)

    result.agent.status = 'running'
    agentEvents(result.ctx, result.agent).emit('agent/status', { status: 'running' })
    result.terminal.send('/clear Must not replace the title while active')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain(
        'Finish or cancel active work before starting a new conversation.',
      )
    })
    expect(result.session.events.filter(event => event.type === 'session/title')).toHaveLength(titleCount)
    expect(swap).not.toHaveBeenCalled()
    ...
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain(
        'session title must contain visible characters. Usage: /clear [name]',
      )
    })
    expect(swap).not.toHaveBeenCalled()

    result.terminal.send('/clear   Archived\t conversation  ')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(swap).toHaveBeenCalledOnce() })
    expect(titlesAtSwap).toEqual(['Archived conversation'])
    expect(foldSessionTitle(result.session.events)).toMatchObject({
      title: 'Archived conversation',
      messageSeqs: [],
      source: { kind: 'user' },
    })
    const clearRuns = result.session.events.filter(event =>
      event.type === 'command/run' && event.data.name === 'clear')
    expect(clearRuns.at(-1)?.data).not.toHaveProperty('args')

    result.terminal.send('/new   Alias-named conversation  ')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(swap).toHaveBeenCalledTimes(2) })
    expect(titlesAtSwap.at(-1)).toBe('Alias-named conversation')
    expect(result.session.events.findLast(event => event.type === 'command/run')?.data)
      .not.toHaveProperty('args')

    result.terminal.send('/reset')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(swap).toHaveBeenCalledTimes(3) })
    expect(titlesAtSwap.at(-1)).toBe('Alias-named conversation')
    await dispose(result)
  })

  it('reports bare rename without human history and explicit rename without a title service', async () => {
    ...
    empty.terminal.send('/rename')
    empty.terminal.send('\r')
    await vi.waitFor(() => {
      expect(empty.terminal.output).toContain(
        'Cannot generate a session name before the conversation has a human message.',
      )
    })
    expect(foldSessionTitle(empty.session.events)).toBeUndefined()
    await dispose(empty)

    const unavailable = await setup()
    unavailable.terminal.send('/rename unavailable')
    unavailable.terminal.send('\r')
    await vi.waitFor(() => {
      expect(unavailable.terminal.output).toContain(
        'Session renaming is unavailable because this runtime has no session-title service.',
      )
    })
    expect(foldSessionTitle(unavailable.session.events)).toBeUndefined()
    await dispose(unavailable)
  })

  it('toggles the task checklist while retaining hidden updates and the editor draft', async () => {
    const result = await setup({
      beforeMount(session) {
        session.append('todo/write', {
          todos: [{ content: 'Initial checklist item', status: 'in_progress' }],
        })
      },
    })

    expect(result.terminal.output).toContain('Initial checklist item')
    const eventCount = result.session.events.length
    const hiddenOutput = result.terminal.output.length
    result.terminal.send('\x14')
    await tick()
    expect(result.session.events).toHaveLength(eventCount)
    expect(result.terminal.output.slice(hiddenOutput)).not.toContain('Initial checklist item')

    const hiddenUpdateOutput = result.terminal.output.length
    result.session.append('todo/write', {
      todos: [{ content: 'Latest hidden checklist item', status: 'pending' }],
    })
    await tick()
    expect(result.terminal.output.slice(hiddenUpdateOutput)).not.toContain('Latest hidden checklist item')

    const restoredOutput = result.terminal.output.length
    result.terminal.send('\x14')
    await tick()
    expect(result.terminal.output.slice(restoredOutput)).toContain('Latest hidden checklist item')

    result.terminal.send('draft survives')
    result.terminal.send('\x14')
    result.terminal.send('\x14')
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'draft survives' }])
    await dispose(result)
  })

  it('stashes, replaces, and restores a prompt with its undo history without logging the action', async () => {
    const result = await setup()
    const eventCount = result.session.events.length

    result.terminal.send('\x13')
    expect(result.session.events).toHaveLength(eventCount)
    result.terminal.send('first draft')
    result.terminal.send('\x1b[D')
    result.terminal.send('\x13')
    await tick()
    expect(result.terminal.output).toContain('Prompt stashed · Ctrl+S to restore')
    expect(result.session.events).toHaveLength(eventCount)

    for (const character of 'alpha beta') result.terminal.send(character)
    result.terminal.send('\x13')
    result.terminal.send('\x13')
    result.terminal.send('\x1f')
    result.terminal.send('Z')
    result.terminal.send('\r')
    expect(result.agent.sent).toEqual([[{ type: 'text', text: 'alphaZ' }]])
    await dispose(result)
  })

  it('restores large pasted content and its cursor instead of submitting a literal paste marker', async () => {
    const result = await setup()
    const pasted = 'p'.repeat(1_001)
    result.terminal.send('pre')
    result.terminal.send(`\x1b[200~${pasted}\x1b[201~`)
    result.terminal.send('post')
    for (let index = 0; index < 4; index += 1) result.terminal.send('\x1b[D')
    result.terminal.send('\x13')
    await tick()
    expect(result.terminal.output).toContain('Prompt stashed · Ctrl+S to restore')
  ...
  it('expires an idle-exit confirmation and clears a later one when work starts', async () => {
    const result = await setup()

    result.terminal.send('\x04')
    await tick()
    expect(result.terminal.output).toContain('Press Ctrl+D again to exit')
    const armedOutput = result.terminal.output.length
    await new Promise(resolve => setTimeout(resolve, 850))
    expect(result.exit).not.toHaveBeenCalled()
    expect(result.terminal.output.slice(armedOutput)).not.toContain('Press Ctrl+D again to exit')
  ...
  it('opens shortcut help only from an empty prompt and opens the model selector with Alt+P', async () => {
    const result = await setup()

    const helpOutput = result.terminal.output.length
    result.terminal.send('?')
    await tick()
    expect(result.terminal.output.slice(helpOutput)).toContain('Keyboard shortcuts')
    expect(result.terminal.output.slice(helpOutput)).toContain('Alt+P choose model')
    result.terminal.send('x')
    await tick()
    expect(result.terminal.output).toContain('Type ? in an empty prompt to toggle this panel')
    result.terminal.send('?')
    await tick()

    result.terminal.send('why?')
    result.terminal.send('\r')
    expect(result.agent.sent).toContainEqual([{ type: 'text', text: 'why?' }])

    const modelOutput = result.terminal.output.length
    result.terminal.send('\x1bp')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(modelOutput)).toContain('Select model')
    })
    result.terminal.send('\x1b')
    await tick()
  ...
  it('runs a leading-bang command through the shell seam and sends durable result context', async () => {
    const result = await setup({ cwd: '/workspace/direct-shell' })
    const env = Object.freeze({ DSH_SHELL: '1', DSH_SESSION_ID: 'main-session' })
    const collect = vi.fn((_execution: ToolExecutionInput) => env)
    const policy = Object.freeze({
      mode: 'workspace-write' as const,
      workspaceRoot: '/workspace/direct-shell',
      sessionId: 'main-session',
    })
    const resolvePolicy = vi.fn(() => policy)
    const resolveShell = vi.fn((request: ShellExecRequest) => resolvedShellSpec(request))
    const process = new ControllableShellProcess()
    const startShell = vi.fn((_spec: ShellExecSpec) => process)
    result.ctx.provide('shellEnv', { collect } as never)
    result.ctx.provide('sandboxPolicy', { resolve: resolvePolicy } as never)
    result.ctx.provide('shell', {
      sandboxMode: 'workspace-write',
      resolve: resolveShell,
      start: startShell,
    } as never)

    result.terminal.send('! node scripted.js')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(1) })
    process.push('DIRECT_STDOUT\n[stderr]\nDIRECT_STDERR\n')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('DIRECT_STDOUT') })
    process.finish(7)
    await vi.waitFor(() => { expect(result.agent.sentMessages).toHaveLength(1) })

    expect(result.terminal.output).toContain('Shell · running $ node scripted.js')
    expect(resolvePolicy).toHaveBeenCalledWith({ session: result.session })
    const execution = collect.mock.calls[0]?.[0]
    expect(execution?.name).toBe('user-shell')
    expect(execution?.arguments).toEqual({ command: 'node scripted.js' })
    expect(execution?.agent).toBe(result.agent)
    expect(execution?.signal).toBeInstanceOf(AbortSignal)
    const shellRequest = resolveShell.mock.calls[0]?.[0]
    expect(shellRequest).toMatchObject({
      command: 'node scripted.js',
      workdir: '/workspace/direct-shell',
      dshEnv: env,
      sandboxPolicy: policy,
    })
    expect(shellRequest?.signal).toBeInstanceOf(AbortSignal)
    expect(startShell).toHaveBeenCalledWith(expect.objectContaining({
      command: 'node scripted.js',
      workdir: '/workspace/direct-shell',
    }))
    expect(result.agent.sentMessages[0]?.source).toMatchObject({
      kind: 'plugin',
      plugin: 'user-shell',
      form: 'notice',
      summary: '$ node scripted.js · exit code 7',
    })
    expect(result.agent.sentMessages[0]?.content).toHaveLength(1)
    const shellContext = result.agent.sentMessages[0]?.content[0]
    expect(shellContext?.type).toBe('text')
    expect(shellContext?.type === 'text' && shellContext.text).toContain('DIRECT_STDOUT\n')
    expect(shellContext?.type === 'text' && shellContext.text).toContain('DIRECT_STDERR\n')
    expect(shellContext?.type === 'text' && shellContext.text).toContain('status: exit code 7')
    await dispose(result)
  })

  it('completes a prior project shell command with Tab and executes the exact recalled line', async () => {
    const first = new ControllableShellProcess()
    const second = new ControllableShellProcess()
    const processes = [first, second]
    let processIndex = 0
    const startShell = vi.fn((_spec: ShellExecSpec) => {
      const process = processes[processIndex]
      processIndex += 1
      if (process === undefined) throw new Error('unexpected extra shell process')
      return process
    })
    const result = await setup({
      cwd: '/workspace/direct-shell-history',
      configureContext: async (ctx) => {
        await ctx.plugin(SystemPrompt)
        await ctx.plugin(ToolRegistry)
        ctx.provide('shell', {
          resolve: resolvedShellSpec,
          start: startShell,
        } as never)
      },
    })

    result.terminal.send('! pnpm test --filter tui')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(1) })
    first.finish()
    await vi.waitFor(() => { expect(result.agent.sentMessages).toHaveLength(1) })

    result.terminal.send('! pnpm t')
    result.terminal.send('\t')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('! pnpm test --filter tui')
    })
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(2) })
    expect(startShell.mock.calls[1]?.[0].command).toBe('pnpm test --filter tui')
    second.finish()
    await dispose(result)
  })

  it('opens live forward-slash path completion in shell input and executes the selected path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'dsh-shell-path-autocomplete-'))
    await mkdir(join(cwd, 'scripts'))
    await writeFile(join(cwd, 'scripts', 'run-me.ps1'), 'Write-Output ok')
    const process = new ControllableShellProcess()
    const startShell = vi.fn((_spec: ShellExecSpec) => process)
    const result = await setup({
      cwd,
    ...
    try {
      result.terminal.send('! ./scripts')
      result.terminal.send('/')
      await vi.waitFor(() => {
        expect(result.terminal.output).toContain('run-me.ps1')
      })
      result.terminal.send('\t')
      await vi.waitFor(() => {
        expect(result.terminal.output).toContain('! ./scripts/run-me.ps1')
      })
      result.terminal.send('\r')
      await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(1) })
      expect(startShell.mock.calls[0]?.[0].command).toBe('./scripts/run-me.ps1')
      process.finish()
    } finally {
      await dispose(result)
  ...
  it('keeps invalid or unavailable shell input editable instead of sending it to the model', async () => {
    const result = await setup()

    result.terminal.send('!')
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output).toContain('Usage: ! <command>')
    expect(result.agent.sentMessages).toHaveLength(0)

    result.terminal.send('\x03')
    result.terminal.send('! echo unavailable')
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output).toContain('Shell mode is unavailable because this runtime has no shell executor.')
    expect(result.terminal.output).toContain('! echo unavailable')
    expect(result.agent.sentMessages).toHaveLength(0)
    await dispose(result)
  })

  it('lets Ctrl+C cancel the foreground shell before it can wake the model', async () => {
    const result = await setup()
    let capturedSpec: ShellExecSpec | undefined
    const process = new ControllableShellProcess()
    process.push('partial\n')
    const startShell = vi.fn((spec: ShellExecSpec) => {
      capturedSpec = spec
      return process
    })
    result.ctx.provide('shell', {
    ...
    result.terminal.send('! long-running-command')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(1) })
    result.terminal.send('\x04')
    await tick()
    expect(result.terminal.output).toContain('Background or cancel the foreground shell command before exiting.')
    expect(result.exit).not.toHaveBeenCalled()

    result.terminal.send('\x03')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Shell command cancelled: $ long-running-command')
    })
    expect(capturedSpec?.signal?.aborted).toBe(true)
    expect(process.killCalls).toBe(1)
    expect(result.agent.sentMessages).toHaveLength(0)
    expect(result.agent.steeredOptions).toHaveLength(0)
    expect(result.exit).not.toHaveBeenCalled()
    await dispose(result)
  })

  it('promotes a live shell to jobs without stealing output or duplicating completion delivery', async () => {
    const process = new ControllableShellProcess()
    const result = await setup({
      config: { directShellOutputRefreshMs: 5 },
      configureContext: async (ctx) => {
        await ctx.plugin(SystemPrompt)
        await ctx.plugin(ToolRegistry)
    ...
    result.terminal.send('! node streaming.js')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(startShell).toHaveBeenCalledTimes(1) })
    process.push('EARLY_OUTPUT\n')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('EARLY_OUTPUT') })

    result.terminal.send('\x02')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Shell moved to background as bash-1')
    })
    expect(result.ctx.jobs.get(JobId('bash-1'), result.agent)).toMatchObject({
      status: 'running',
      reported: true,
      label: 'node streaming.js',
    })

    result.terminal.send('/tasks')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Background tasks')
      expect(result.terminal.output).toContain('bash-1 · running · node streaming.js')
    })
    expect(result.ctx.jobs.read(JobId('bash-1'), result.agent).text).toContain('EARLY_OUTPUT')

    process.push('LATE_OUTPUT\n')
    process.finish()
    await vi.waitFor(() => { expect(result.agent.sentMessages).toHaveLength(1) })

    const completion = result.agent.sentMessages[0]
    expect(completion?.source).toMatchObject({ kind: 'plugin', plugin: 'user-shell' })
    const completionText = completion?.content[0]
    expect(completionText?.type === 'text' && completionText.text).toContain('EARLY_OUTPUT\nLATE_OUTPUT\n')
    await vi.waitFor(() => {
      expect(result.ctx.jobs.get(JobId('bash-1'), result.agent)).toMatchObject({
        status: 'completed',
        reported: true,
  ...
  it('combines session autocomplete with files and prepares send/steer references asynchronously', async () => {
    ...
    // Idle: the snapshot rides the prompt's admission (the pre-step decision's messages on
    ...
    const decision = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step', { messages: [result.agent.sentMessages[0]!], turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter', messages: [result.agent.sentMessages[0]!] }),
    )
    expect(decision.kind).toBe('enter')
    expect(decision.kind === 'enter' && decision.messages[1]?.source)
    ...
    expect(replay.kind === 'enter' && replay.messages.length).toBe(1)
  ...
  it('steers a referenced prompt beside its snapshot while running', async () => {
    const result = await setup({
      status: 'running',
      omitInitialLifecycle: true,
      async configureContext(ctx) {
    ...
    // Steering bypasses prompt admission: the snapshot rides the steered
    // message's step boundary through the outbox.
    expect(result.agent.sent).toHaveLength(0)
    expect(result.agent.injected).toHaveLength(1)
    expect(result.agent.injectedOptions[0]?.source).toMatchObject({
      kind: 'session-reference',
      references: [{ sessionId: 'admission-src' }],
    })
    await dispose(result)
  })
  ...
  it('releases the reference-admission wrapper on the ordinary allowed path', async () => {
    ...
    for (const sent of result.agent.sentMessages) {
      await agentEvents(result.ctx, result.agent).waterfall(
        'agent/pre-step', { messages: [sent], turn: 1, step: 1, signal: new AbortController().signal },
        () => Promise.resolve({ kind: 'enter', messages: [sent] }),
      )
    }
    ...
    result.ctx.emit('agent/inbox/discarded', { agent: result.agent, message: result.agent.sentMessages[0]! })
    const unrelated = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step', { messages: [createUserMessage({
        content: [{ type: 'text', text: 'unrelated' }],
        source: { kind: 'user' },
      })], turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter', messages: [createUserMessage({
        content: [{ type: 'text', text: 'unrelated' }],
        source: { kind: 'user' },
      })] }),
    )
    expect(unrelated.kind === 'enter' && unrelated.messages.length).toBe(1)
  ...
  it('releases the reference wrapper when enqueue synchronously discards before followup returns', async () => {
    ...
    result.agent.followup = (input) => {
      ...
      result.ctx.emit('agent/inbox/inserted', { agent: foreign, message })
      result.ctx.emit('agent/inbox/inserted', { agent: result.agent, message })
      result.ctx.emit('agent/inbox/discarded', { agent: result.agent, message })
      return message.id
    }
  ...
  it('discards the reference snapshot with its blocked or cancelled prompt', async () => {
    ...
    result.ctx.on('agent/pre-step', async ({ messages }, next) =>
      blockPrompts && messages.length === 1 ? { kind: 'reject' as const } : next())
    ...
    expect(passthrough.kind === 'enter' && passthrough.messages.length).toBe(1)
    // A foreign agent's discard leaves the wrapper armed.
    const foreign = { ...result.agent, id: SessionId('foreign') } as unknown as Agent
    result.ctx.emit('agent/inbox/discarded', { agent: foreign, message: result.agent.sentMessages.at(-1)! })
    // An unrelated discard for this agent also leaves the wrapper armed.
    result.ctx.emit('agent/inbox/discarded', { agent: result.agent, message: createUserMessage({
      content: [{ type: 'text', text: 'unrelated discard' }],
      source: { kind: 'user' },
    }) })
    result.ctx.emit('agent/inbox/discarded', { agent: result.agent, message: result.agent.sentMessages.at(-1)! })
    await tick()
    ...
    const afterDiscard = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step', { messages: [result.agent.sentMessages.at(-1)!], turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter', messages: [result.agent.sentMessages.at(-1)!] }),
    )
    expect(afterDiscard.kind === 'enter' && afterDiscard.messages.length).toBe(1)
    await dispose(result)
  })
  ...
  it('fuzzy-completes files and directories while sending only the selected path text', async () => {
    ...
    // Windows rejects control characters in filenames, so the unsafe-name
    // fixture (and its negative completion assertion below) only exists on
    // POSIX; on Windows no such candidate can be indexed, which is the same
    // observable outcome the assertion guards.
  ...
  it('controls adapter-advertised reasoning effort with /effort and Alt+T without losing the draft', async () => {
    const result = await setup({
      initialModelSelection: {
        provider: 'alpha',
        model: 'think',
      ...
      catalog: {
        ...
        models: [
          { provider: 'alpha', id: 'think', name: 'Thinking Model' },
          { provider: 'alpha', id: 'always', name: 'Always Thinking' },
          { provider: 'alpha', id: 'disabled', name: 'Thinking Disabled' },
          { provider: 'alpha', id: 'plain', name: 'Plain Model' },
        ],
        async resolveModelInfo(_provider, model) {
          const efforts: Record<string, LlmModelReasoningInfo | undefined> = {
            think: {
              efforts: [
                { id: ReasoningEffortId('off'), name: 'Off' },
                { id: ReasoningEffortId('high'), name: 'High', description: 'Fast reasoning' },
                { id: ReasoningEffortId('max'), name: 'Max' },
              ],
              defaultEffort: ReasoningEffortId('high'),
            },
          ...
          return {
            ...
            ...efforts[model] === undefined ? {} : { reasoning: efforts[model] },
          }
        },
      },
    })
    ...
    result.terminal.send('draft')
    const firstToggle = result.terminal.output.length
    result.terminal.send('\x1bt')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(firstToggle)).toContain('Reasoning effort: Off. New steps will use it.')
    })
    expect(result.agent.sent).toEqual([])
    expect(result.agent.steered).toEqual([])

    const restoredToggle = result.terminal.output.length
    result.terminal.send('\x1bt')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(restoredToggle)).toContain('Reasoning effort: Max. New steps will use it.')
    })
    result.terminal.send('!')
    result.terminal.send('\r')
    await tick()
    expect(result.agent.sent).toEqual([[{ type: 'text', text: 'draft!' }]])

    result.agent.status = 'running'
    result.ctx.emit('agent/status', { agent: result.agent, status: 'running' })
    const autoOutput = result.terminal.output.length
    result.terminal.send('/effort auto')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(autoOutput)).toContain('Reasoning effort: Auto (adapter default: High). New steps will use it.')
    })
    await result.ctx.systemPrompt.assemble(assembleContextFor(result.agent))
    ...
    )).resolves.toEqual({ provider: 'alpha', model: 'think' })

    result.terminal.send('/effort auto')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Reasoning effort is already Auto (adapter default: High) for alpha/think.')
    })
    result.terminal.send('/effort impossible')
    result.terminal.send('\r')
    result.terminal.send('/effort high extra')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Available for alpha/think: auto, off, high, max.')
    })

    const pickerOutput = result.terminal.output.length
    result.terminal.send('/effort')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      const output = result.terminal.output.slice(pickerOutput)
      expect(output).toContain('Reasoning effort')
      expect(output).toContain('Model: alpha/think')
    ...
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(pickerOutput)).toContain('Reasoning effort: High. New steps will use it.')
    })
    result.terminal.send('\x1bt')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Reasoning effort: Off.') })
    const restoreHighOutput = result.terminal.output.length
    result.terminal.send('\x1bt')
    await vi.waitFor(() => {
      expect(result.terminal.output.slice(restoreHighOutput)).toContain('Reasoning effort: High.')
    })

    result.terminal.send('/model alpha/always')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Model selected: alpha/always.') })
    result.terminal.send('\x1bt')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('alpha/always cannot disable extended thinking')
      expect(result.terminal.output).toContain('"off" effort.')
    })

    result.terminal.send('/model alpha/disabled')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Model selected: alpha/disabled.') })
    result.terminal.send('\x1bt')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('alpha/disabled cannot enable extended thinking')
      expect(result.terminal.output).toContain('adapter advertises no enabled')
    })

    result.terminal.send('/model alpha/plain')
    result.terminal.send('\r')
    await vi.waitFor(() => { expect(result.terminal.output).toContain('Model selected: alpha/plain.') })
    result.terminal.send('/effort')
    result.terminal.send('\r')
    await vi.waitFor(() => {
      expect(result.terminal.output).toContain('Reasoning effort is unavailable for alpha/plain')
    })
    expect(result.agent.sent).toHaveLength(1)
    expect(result.agent.steered).toEqual([])
    await dispose(result)
  ...
  it('cancels before /quit while running and handles agent errors/disposal', async () => {
    ...
    // Disposal now reports the agent by identity instead of a generic stop line.
    expect(events.terminal.output).toContain('Agent "main-session" was disposed.')
...
describe('tool cards and surface replay', () => {
  ...
  it('keeps append-origin history and marks a landed compaction, live and on rebuild', async () => {
    ...
    const checkpoint = result.session.append('user/message', createUserMessage({
      ...
      source: compactCheckpointSource(CompactionId('spec-checkpoint')),
    }), {
    ...
    // Changing reasoning display rebuilds the transcript from the log; the
  ...
  it('keeps accepted user input ahead of the assistant when step/start is logged first', async () => {
    ...
    appendUser(result.session, 'ordered prompt')
    appendAssistant(result.session, [{ type: 'text', text: 'ordered answer' }])
    result.session.append('step/end', { turn: 1, step: 1 })
...
describe('TUI user-interaction dialogs', () => {
  ...
  it('answers single-select, multi-select, custom, and optionless questions', async () => {
    ...
    result.terminal.send('\t')
    await tick()
    expect(result.terminal.output).toContain('2 selected • Ctrl+G editor • Enter submit • Esc options')
  ...
  it('edits a multiline custom question response with the same external-editor lifecycle', async () => {
    const editText = vi.fn<NonNullable<TuiRuntime['editText']>>()
    ...
    const answer = result.ctx.userQuestions.ask({
      questions: [{
        id: 'external-custom',
        question: 'Describe the alternative',
        options: [{ label: 'Default' }],
      }],
    })
    await tick()

    // Ctrl+G from option mode enters the custom-response editor directly.
    result.terminal.send('\x07')
    await vi.waitFor(() => { expect(editText).toHaveBeenCalledWith({ draft: '' }) })
    await vi.waitFor(() => { expect(result.terminal.started).toBe(2) })
    result.terminal.send('\r')
    await expect(answer).resolves.toEqual({
      answers: [{ id: 'external-custom', selected: [], custom: 'first line\nsecond line' }],
    })
    expect(result.terminal.stopped).toBe(1)
    await dispose(result)
  })
  ...
  it('handles option wrapping, deselection errors, and returning from custom input', async () => {
    ...
    await tick()
    expect(result.terminal.output).toContain('0 selected • Ctrl+G editor • Enter submit • Esc options')
    result.terminal.send('\x1b')
    await tick()
...
describe('terminal mounting', () => {
  it('starts immediately when the configured agent already exists', async () => {
    ...
    ctx.agents.register({
      ...
      followup: () => {}, steer: () => {}, inject: () => {}, send: () => {}, cancel() {}, whenIdle: () => Promise.resolve(), runMaintenance() { throw new Error('spec: runMaintenance is not supported') },
    })
    const terminal = new FakeTerminal()
  ...
  it('degrades /reload to a warning when mounted as a real plugin without a Loader', async () => {
    ...
    const terminal = new FakeTerminal()
    // Mirror dsh-tui's own inject (minus loader, the absence under test). An
    // inject fiber stays PENDING until every named service resolves, so the
    // mirror must name exactly the services mounted above — a stale name (or a
    // missing `sessions`/`systemPrompt` provider) silently never loads apply.
    await ctx.plugin(SystemPrompt)
    await ctx.plugin({
      inject: ['agents', 'sessions', 'commands', 'userQuestions', 'tools', 'llm', 'systemPrompt', 'tokenMeter', 'tuiPrompt'],
  ...
  it('waits for its configured agent before starting the TUI', async () => {
    ...
    ctx.agents.register({
      ...
      inbox: new Inbox(otherSession, { inserted() {}, discarded() {}, claimed() {} }),
      followup: () => {}, steer: () => {}, inject: () => {}, send: () => {}, cancel() {}, whenIdle: () => Promise.resolve(), runMaintenance() { throw new Error('spec: runMaintenance is not supported') },
    })
    expect(terminal.started).toBe(0)
