...
import { createUserMessage, CallId, type ContentBlock , createMessage, createToolResultMessage, ReasoningEffortId } from '@deepseek-ai/dsh-llm'
...
const CHECKPOINTS = [
  ...
  'todo-checklist-hidden',
  'todo-checklist-restored',
  'todo-plan-cleared',
] as const
...
function appendCompactionCheckpoint(session: Session, range: CompactionRange): void {
  session.append('user/message', createUserMessage({
    ...
    source: compactCheckpointSource(CompactionId('fixture-checkpoint')),
  }), {
...
describe('TUI terminal-state snapshots', () => {
  ...
  it('hides and restores the latest task checklist with Ctrl+T', async () => {
    const harness = await setupSnapshot({
      beforeMount(session) {
        session.append('todo/write', {
          todos: [
    ...
    await renderAfter(harness, () => {
      harness.terminal.send('\x14')
    })
    await checkpoint('todo-checklist-hidden', harness.terminal)
    harness.session.append('todo/write', {
      todos: [
        { content: 'inspect the workspace', status: 'completed' },
        { content: 'verify the latest change', status: 'in_progress' },
        { content: 'ship the result', status: 'pending' },
      ],
    })
    await renderAfter(harness, () => {
      harness.terminal.send('\x14')
    })
    await checkpoint('todo-checklist-restored', harness.terminal)
    await disposeSnapshot(harness)
  })
  ...
  it('pins an explicit durable session rename in the banner and command transcript', async () => {
    const harness = await setupSnapshot({
      async configureContext(ctx) {
    ...
    harness.terminal.send('/rename   Release\t readiness review  ')
    harness.terminal.send('\r')
    await vi.waitFor(async () => {
      expect(await harness.terminal.snapshot({ includeScrollback: true }))
        .toContain('Session renamed: Release readiness review.')
    })
    await checkpoint('session-renamed', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })
  ...
  it('pins the persisted live-settings selector after a committed toggle', async () => {
    ...
    await renderAfter(harness, () => {
      harness.terminal.send('/config')
      harness.terminal.send('\r')
    })
    const beforeToggle = harness.terminal.frames
    harness.terminal.send('\x1b[B')
    harness.terminal.send('\r')
    await vi.waitFor(() => {
      expect(settings.externalEditorContext).toBe(true)
      expect(harness.terminal.frames).toBeGreaterThan(beforeToggle)
    })
    await harness.terminal.flush()
    await checkpoint('tui-settings', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })
  ...
  it('pins help, unknown commands, live errors, turn failures, and terminal restoration', async () => {
    ...
    await renderAfter(harness, () => {
      ...
      harness.session.append('turn/end', { turn: 3, reason: { kind: 'blocked' } })
  ...
  it('pins the adapter-authoritative reasoning-effort selector', async () => {
    const harness = await setupSnapshot({
      initialModelSelection: { provider: 'alpha', model: 'think' },
      catalog: {
        providers: [{ id: 'alpha', name: 'Alpha' }],
        models: [{ provider: 'alpha', id: 'think', name: 'Thinking Model' }],
    ...
    await renderAfter(harness, () => {
      harness.terminal.send('/effort')
      harness.terminal.send('\r')
    })
    await checkpoint('reasoning-effort-selector', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })

  it('pins the assistant-response code-block copy selector', async () => {
    const harness = await setupSnapshot({
  ...
  it('pins empty-prompt shortcut help and the first idle-exit confirmation', async () => {
    const harness = await setupSnapshot({}, { columns: 92, rows: 32 })
    await renderAfter(harness, () => {
      harness.terminal.send('?')
    })
    await checkpoint('shortcut-help', harness.terminal, { includeScrollback: true })
    await renderAfter(harness, () => {
      harness.terminal.send('?')
    })
    await renderAfter(harness, () => {
      harness.terminal.send('\x04')
    })
    await checkpoint('idle-exit-confirmation', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })

  it('pins the first stop-all-background-subagents chord confirmation', async () => {
    const harness = await setupSnapshot({}, { columns: 92, rows: 32 })
    harness.ctx.provide('jobs', {
      list: () => [],
      kill: () => 'already-finished',
    } as never)
    await renderAfter(harness, () => {
      harness.terminal.send('\x18\x0b')
    })
    await checkpoint('subagent-stop-confirmation', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })

  it('pins a multiline draft after returning from an external editor', async () => {
    const harness = await setupSnapshot({
      editText: () => Promise.resolve('edited externally\nwith a second line'),
    }, { columns: 92, rows: 32 })
    const before = harness.terminal.frames
    harness.terminal.send('old draft')
    harness.terminal.send('\x07')
    await vi.waitFor(() => { expect(harness.terminal.started).toBe(2) })
    await vi.waitFor(() => { expect(harness.terminal.frames).toBeGreaterThan(before) })
    await checkpoint('external-editor-returned', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })

  it('pins Ctrl+L redraw confirmation without clearing the current draft', async () => {
    const harness = await setupSnapshot({}, { columns: 92, rows: 32 })
    await renderAfter(harness, () => {
      harness.terminal.send('draft survives redraw')
      harness.terminal.send('\x0c')
    })
    await checkpoint('clear-confirmation', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })

  it('stashes and restores a large paste at the same cursor position', async () => {
    const harness = await setupSnapshot({}, { columns: 92, rows: 32 })
    await renderAfter(harness, () => {
      harness.terminal.send('prefix ')
      harness.terminal.send(`\x1b[200~${'p'.repeat(1_001)}\x1b[201~`)
      harness.terminal.send(' suffix')
      for (let index = 0; index < 7; index += 1) harness.terminal.send('\x1b[D')
      harness.terminal.send('\x13')
    })
    await checkpoint('prompt-stashed', harness.terminal)
    await renderAfter(harness, () => {
      harness.terminal.send('\x13')
    })
    await checkpoint('prompt-restored', harness.terminal)
    await disposeSnapshot(harness)
  })
  ...
  it('pins the expanded context-pressure and composition card', async () => {
    const harness = await setupSnapshot({
    ...
    await renderAfter(harness, () => {
      harness.terminal.send('/context all')
      harness.terminal.send('\r')
    })
    await checkpoint('context-usage', harness.terminal, { includeScrollback: true })
    await disposeSnapshot(harness)
  })
