...
- `start(spec): JobId` 验证已附加的任务控制器、spec、确切且仍存活的 owner、可选的正数 `outputLimitBytes`，以及 Service Provider 所拥有的准入策略，然后只调用生产方的 `run()` 一次。`run()` 可以启动工作，也可以移交一项由生产方持有的存活资源。预检拒绝时不会生成 job id，也绝不会调用 `run()`，因此执行移交的生产方仍持有该资源；启动方抛出异常时不会注册工作，成功返回则直接提交，不再执行其他可能失败的步骤。
...
`completionDelivery` 声明由谁持有终止结果交付。默认值 `registry` 允许已附加的完成监听器报告一项尚未被认领的结算；`producer` 则是显式承诺：生产方会自行交付结果。后一种记录从 `reported: true` 开始，因此通用完成报告方无法重复添加模型输入。该标记不会改变输出读取、生命周期或取消语义。
...
- **转入后台需要生产方协作，注册表不会自行发现**：仍持有存活句柄的生产方可以在预检后通过 `start()` 移交它；注册表无法发现或夺取任意前台工作，预检拒绝时生产方必须继续持有资源。
