...
| `@deepseek-ai/dsh-tool-allin` | `allin` | `ctx.tools`、`ctx.workflowEngine`、`ctx.subagents`、`ctx.systemPrompt`、`a calling Agent (exec.agent parents every child)` | `tool/call`、`tool/result`、`workflow and child session events during execution` | - | 固定的前台工作流会把一个目标编译为顶层任务，按波次并行运行依赖已就绪的通道，并返回协调器综合结果；模型只能选择目标和可选的任务上限。 |
...
把一个大型具体目标编译为相互独立的顶层任务，在并行的 flash 通道中运行每个依赖已就绪的任务，并返回 pro 协调器综合结果。仅用于明确的 allinluna 式多 agent 请求，或一个横跨相互独立工作领域的大型目标，而不是例行的单轮次工作。规划与综合使用 pro 模型；任务通道使用 flash，并同时运行多条通道。调用会等待整个运行结束。
...
来源：[`packages/workflow/tool-allin/src/index.ts`](../packages/workflow/tool-allin/src/index.ts)

固定的前台工作流会把一个目标编译为顶层任务，按波次并行运行依赖已就绪的通道，并返回协调器综合结果；模型只能选择目标和可选的任务上限。
