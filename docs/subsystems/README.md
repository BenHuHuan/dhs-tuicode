...
| [tui.md](tui.md) | the interactive terminal front door: overlay admission, prompt-value updates, and runner-owned live-agent publication |
...
| [mcp.md](mcp.md) | the redacted MCP connection directory: public server names, transport, lifecycle state, tool names, and its read-only `ctx.mcpConnections` service |
