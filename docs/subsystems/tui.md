...
The TUI subsystem owns the interactive terminal front door. Its package [README](../../packages/ui/tui/README.md) documents user-visible rendering, commands, configuration, terminal restoration, and model effects; this page owns only the Cordis services and event used to assemble that front door.

`ctx.tui` admits lifecycle-bound overlay requests after terminal startup. `ctx.tuiPrompt` supplies mutable prompt-template values to the renderer. The app-owned runner publishes its current interactive agent through `ctx.tuiAgent` and emits `tui-agent/ready` after a create, resume, or committed session swap.
