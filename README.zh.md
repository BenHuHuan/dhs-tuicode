...
- **自适应 Router 后续轮次：** 同步可复现的 mode-boost v0.1 行为：问候与短聊天不施加路由压力；第三轮起的新任务重新分类；按任务复杂度选择引导；Flash 模型不附加额外的推理收束约束。
...
- **Router Standard（`/mode router`）：** 移植 [dsh-routing-suite](https://github.com/yjh051108/dsh-routing-suite) 当前可复现的 v0.2.0 RL 接口还原：首轮只携带 RL 训练句 + shell/str_replace_editor 工具面，第一次工具调用后恢复完整目录。
...
两个 Router Profile 还同步了该套件可复现的 mode-boost v0.1 改进：短对话不添加路由引导；第三轮及之后遇到新任务时重新分类；简单任务使用 commit-and-act 引导，复杂任务强调架构、边界条件和集成点；明确的推理收束后缀仅用于非 Flash 模型。
