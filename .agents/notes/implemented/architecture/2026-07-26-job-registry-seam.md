...
The later TUI direct-shell integration extended this seam without moving the package boundary. `JobStart.run()` may now transfer a producer-owned resource that is already live; all registry admission still happens first, so rejection never calls `run()` and leaves that resource with its producer. `JobStart.completionDelivery` also distinguishes registry-owned completion notices from a producer's explicit commitment to deliver richer terminal context. The local provider initializes producer-delivered records as reported, preventing the generic `dsh-tool-jobs` listener from duplicating that model input. Neither addition exposes process handles or TUI concepts through the Service Definition.
