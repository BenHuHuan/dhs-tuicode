...
> 在终端前端获得具名产品组合与组装验收后，本移除决策已由[恢复已交付的 TUI profile](../feature/2026-08-14-restore-tui-profile.md)逆转。下文关于维护成本与重新引入门槛的论证，仍然是移除期间的决策依据。
...
本记录持有始于 2026-08-04 的移除期间。`packages/ui/tui` 包当时在不提供兼容包或别名的情况下被删除，其源码、包测试、终端快照、依赖声明、已打补丁的 `pi-tui` 产物、workspace 引用、生成的服务目录条目和文档也一并移除。通用宿主能力与 agent loop（智能体循环）能力保持不变。当前代码包与 `tui` profile 由上方链接的恢复决策持有。
...
在移除时点，仓库搜索结果与生成的服务目录中不再包含 TUI 包、依赖补丁、服务键或包链接。常规源码构建、类型检查、lint、hygiene、文档门禁以及其余组装快照测试套件均可在没有已删除 workspace 的情况下运行。当前验证由恢复记录持有。
...
在移除期间，DeepSeek Harness 不提供终端 UI 包。依赖该包的 import 和 `cordis.yml` 条目会直接失败，不会得到兼容转换。Web 是已交付的交互界面；ACP、JSON-RPC 与一次性 CLI 是 Web 之外的入口。

提供方无关的命令、用户交互、审批、工具呈现、PTY 与会话投影能力在移除期间仍可供其他宿主使用。恢复后的 TUI 现在通过具名产品 profile、显式包边界、具体交互提供方，以及组装后的生命周期与 transcript（文本记录）验收，满足了本记录提出的重新引入门槛。
